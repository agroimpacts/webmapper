...
function addBasemap() {
    ...
    center: [-0.5, 7.18], // starting position [lng, lat]
...
function initial(){
    ...
    map.on('load', function(){
        ...
        // adding all 16 vct layers on initial load
        for (i=1;i<17;i++) {
            _addVctSource(i.toString())
            _addVctLayer(i.toString())
        }
    });
...
function _selectAOI(aoiValue) {
    var obj = $( "option:selected" );
    
    if (typeof(aoiValue) === "number") {
        var slctAOI = aoiValue.toString()
    } else{
        var slctAOI = obj.val();
    }
    ...
    if(slctAOI!=currentAOI){
        // check if Comparison button and prob image are on
        // True: clear prob image on mapbrother and reload it with new AOI
        if($('#Prob-CP').prop("checked") && curProblayerID.length!=0){
            // remove prob image
            for(let i=0; i<curProblayerID.length; i++) {
                mapbrother.removeLayer(curProblayerID[i]);
            };
            for(let i=0; i<curProbsourceID.length; i++) {
                mapbrother.removeSource(curProbsourceID[i]);
            };
            curProblayerID = [];
            curProbsourceID = [];
            ...
            // commenting logic to 
            // remove previous layer and add new layer.
            // currently just zooming to center of layer on click and
            //  select events as all layers are already loaded
            ...
            // // reload prob image with new AOI
        ...
        if($('#'+clickedOBJ).prop("checked")) {
            ...
            mapbrother.on('load', function(){
                if($('#Prob').prop("checked")){
                    ...
                    if(map.getLayer(curProblayerID)!=null){
                        for(let i=0; i<curProblayerID.length; i++) {
                            map.removeLayer(curProblayerID[i]);
                        };
                        curProblayerID = [];
                    }
                    // Remove prob imgs source from map
                    if(map.getSource(curProbsourceID)!=null){
                        for(let i=0; i<curProbsourceID.length; i++) {
                            map.removeSource(curProbsourceID[i]);
                        };
                        curProbsourceID = [];
                    }
            ...
            if(mapbrother.getLayer(curProblayerID)!=null){
                for(let i=0; i<curProblayerID.length; i++) {
                    mapbrother.removeLayer(curProblayerID[i]);
                };
                curProblayerID = [];
            ...
            if(mapbrother.getSource(curProbsourceID)!=null){
                for(let i=0; i<curProbsourceID.length; i++) {
                    mapbrother.removeSource(curProbsourceID[i]);
                };
                curProbsourceID = [];
...
function _removeRstSource(clickedfield) {
    // clear the sources stored in curOSsourceID, curGSsourceID, curProbsourceID
    if(clickedfield == 'OS') {
...
function _addRstLayer(aoi, clickedfield) {
    ...
    if(clickedfield == 'OS') {
        ...
        if($('#Prob-CP').prop("checked")){
            for(var i=0; i<curProbsourceID.length; i++) {
                var sourceID = curProbsourceID[i];
                var layerID = 'aoi'+ aoi + '-rst-' + clickedfield + '-' + i + '-layer';
                mapbrother.addLayer({
                    'id': layerID,
                    'type': 'raster',
...
function identifyAOI(){
    ...
    // call _selectAOI on click event on map
    map.on('click', 'ghana-aois-viz', function(e){
        // loading data on click
        if(aois){
            var aoiClicked = e.features[0].properties['grp'];
            _selectAOI(aoiClicked)            
        }
    });
...
// Find the order index of the visuallized vctor layer in the map style
